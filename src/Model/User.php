...
{
    const SESSION = "User";
    const SECRET = "EnterYourSecret_"; //The value of the SECRET constant must be at least 16 characters
    ...
    {
        $sql = new Sql();
        $results = $sql->select("CALL sp_userupdate_save(:iduser, :desperson, :deslogin, :despassword, :desemail, :nrphone, :inadmin)",
        array(
    ...
    public static function getForgot($email)
    {
        ...
        if (count($results) === 0) {
            ...
            if (count($results2) === 0) {
                ...
                $link = "http://www.fmecommerce.com/admin/forgot/reset?code=$code";
                $mailer = new Mailer($data["desemail"], $data["desperson"], "Redefine password - FMecommerce", "forgot",
                array(
                    "name" => $data["desperson"],
                    "link" => $link
                ));
